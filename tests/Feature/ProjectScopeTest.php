...
class ProjectScopeTest extends TestCase {
    ...
    public function test_admin_navigation_nests_product_operations_under_products():void {$admin=User::factory()->create(['is_admin'=>true]);$response=$this->actingAs($admin)->get('/admin');$response->assertOk()->assertSee(['Products','Product Manager','Add Product','Bulk Product Upload','Product Media Manager','Images','Videos','360° Product View','Virtual Try-On','Categories','Collections','Variants','Pages'],false);$content=$response->getContent();$this->assertSame(1,substr_count($content,'class="admin-nav-subgroup" open'));$this->assertSame(1,substr_count($content,'class="admin-nav-subitems"'));$this->assertLessThan(strpos($content,'Product Manager'),strpos($content,'class="admin-nav-subgroup" open'));}
