...
class ProjectScopeTest extends TestCase {
    ...
    public function test_product_manager_renders_the_catalogue_dashboard():void {$admin=User::factory()->create(['is_admin'=>true]);$product=Product::create(['name'=>'Manager Test Cap','slug'=>'manager-test-cap','sku'=>'PM-TEST-001','price'=>34.99,'stock'=>24,'is_new'=>true,'is_active'=>true,'status'=>'active','brand'=>'Emerald Rozalia']);$response=$this->actingAs($admin)->get('/admin/resource/product-manager?q=Manager&tab=published');$response->assertOk()->assertSee(['Product Manager / Published','Manage all products, inventory, pricing, visibility and performance','Product Summary','Quick Actions','Manager Test Cap','PM-TEST-001','Published'],false);$this->assertStringContainsString('class="admin-nav-subgroup" open',$response->getContent());}
