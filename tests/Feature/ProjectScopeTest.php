...
use App\Models\{Address,AuditLog,Category,Company,Conversation,Discount,Inquiry,InventoryMovement,Order,PaymentTransaction,Product,ProductMedia,ProductVariant,ReturnRequest,ShippingMethod,User};
...
class ProjectScopeTest extends TestCase {
    ...
    public function test_add_product_uses_the_product_workflow_and_shared_admin_shell():void {$admin=User::factory()->create(['is_admin'=>true]);$category=Category::create(['name'=>'Caps','slug'=>'caps','is_active'=>true,'sort_order'=>1]);$response=$this->actingAs($admin)->get(route('admin.add-product'));$response->assertOk()->assertSee(['Add New Product','Create a new product and publish it to your website and sales channels.','Basic Information','Media & Gallery','Variants & Inventory','SEO & Content','Publish & Visibility','Product Status','Visibility & Channels','Order Master Categories'])->assertDontSee('Add Record',false);$this->assertMatchesRegularExpression('/class="admin-nav-subgroup"\s+open/',$response->getContent());$payload=['name'=>'Workflow Signature Cap','short_description'=>'A premium cap made in Limerick.','slug'=>'workflow-signature-cap','sku'=>'WF-ADD-001','category_id'=>$category->id,'brand'=>'Emerald Rozalia','tags'=>'Cap, Premium','product_type'=>'simple','tax_class'=>'standard','hs_code'=>'6505.00.30','weight'=>'0.250','length'=>'22','width'=>'18','height'=>'12','description'=>'Detailed product description for the workflow test.','material'=>'Premium cotton twill','care'=>'Spot clean only','meta_title'=>'Workflow Signature Cap','meta_description'=>'Premium Emerald Rozalia cap.','cost_price'=>'14.90','price'=>'29.90','compare_price'=>'39.90','vat_rate'=>'23','currency'=>'EUR','stock'=>'12','status'=>'active','publish_date'=>'2026-09-04T10:30','published_website'=>'1','available_for_sale'=>'1','featured'=>'1','channels'=>['website','franchise_portal'],'order_categories'=>['online','bulk'],'save_action'=>'media'];$response=$this->actingAs($admin)->post(route('admin.add-product.store'),$payload);$product=Product::where('sku','WF-ADD-001')->firstOrFail();$response->assertRedirect(route('admin.media.index',['product_id'=>$product->id]));$this->assertSame('workflow-signature-cap',$product->slug);$this->assertSame('active',$product->status);$this->assertTrue((bool)$product->is_active);$this->assertSame(['Cap','Premium'],$product->product_metadata['tags']);$this->assertSame(['website','franchise_portal'],$product->product_metadata['channels']);$this->assertNotNull($product->published_at);}
