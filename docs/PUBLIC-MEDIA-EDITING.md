...
5. In the relevant block, open the **Approved public media** visual selector, search the approved thumbnails, choose one or more assets, save the page, and publish the revision. Use **Preview** before publishing.
...
Reference screenshots and generic placeholder artwork are not used as public page fallbacks. Assets explicitly registered as approved baseline artwork (for example, the Contact destination) are still delivered through their approved UUID records. All other page blocks render an explicit neutral awaiting-media state until an approved asset is selected; selecting one keeps the section’s layout, links, alt text, responsive behavior, and audit trail intact.
...
The homepage and Contact page now consume the same public shell. To update the shared menu, logo reference, footer columns, social links, or legal links, open **Website & Products → Pages → Shared Site Layout** (`/admin/pages/layouts`). Edit a draft through the grouped visual controls, validate it, submit it for approval, approve it, and activate it. The active public snapshot is then used by every public page that extends the shared site layout.
