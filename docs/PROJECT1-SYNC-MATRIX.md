...
| Communication web/channel path | Public forms create shared web Conversation/Message; cPanel replies use the CommunicationCenter service and after-commit delivery job; signed provider callbacks use the event ledger | Communication Center and communication reports | Shared conversation path, reply idempotency, queue/retry state, provider boundary, signed callback duplicate handling and redaction are feature-tested; providers remain intentionally unconfigured | Wire and verify real email/WhatsApp/chat adapters, durable templates/approvals/follow-ups/alerts, reports/analytics and visual/browser/accessibility proof |
