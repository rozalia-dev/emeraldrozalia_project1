...
class PageManagerController extends Controller
{
    ...
    private function normaliseSectionSettings(array $settings, string $type): array
    {
        ...
        // Every public section uses the approved media library. Raw paths and
        // URLs are never accepted as a second, untracked source.
    ...
    private function approvedMediaUuid(mixed $value): ?string
    {
    ...
    private function approvedMedia(): array
    {
        ...
        $assets = MediaAsset::query()->approvedPublic()->latest('created_at')->get();
        ...
        return $assets->concat($productMedia)->concat($variantMedia)->map(function ($media) use ($resolver): array {
            $fallbackAlt = match (true) {
                $media instanceof ProductMedia => $media->product?->name,
                $media instanceof VariantMedia => $media->variant?->product?->name,
                default => null,
            };
    ...
    public function create(Request $request)
    {
        ...
        return view('admin.pages.builder', compact('page', 'approvedMedia'));
    }
