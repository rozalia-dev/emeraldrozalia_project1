...
class Store extends Model { protected $fillable=['owner_id','name','slug','business_name','business_type','email','phone','description','branding','settings','status']; protected function casts():array{return ['branding'=>'array','settings'=>'array'];} public function products():BelongsToMany{return $this->belongsToMany(Product::class)->withPivot(['price','is_active'])->withTimestamps();} }
