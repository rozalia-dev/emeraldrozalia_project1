...
<div class="tax-page" data-taxonomy-page>
    <header class="tax-head">
        <div><span class="tax-kicker">Website &amp; Products · Categories</span><h1>Country Taxonomy Builder</h1><p>Build SHOP ALL menus as Category → Country → Club → Beanies / Caps / Hats.</p></div>
    ...
    <section class="tax-card">
        <div class="tax-section-title"><div><h2>Create / synchronize a menu path</h2><p class="tax-help">UEFA, FIFA and GAA require a club. Traditional and Heritage use EU countries only and skip the club level.</p></div></div>
        <form method="post" action="{{ route('admin.categories.taxonomy.build') }}" class="tax-grid" data-taxonomy-builder>
            ...
            <label><span>Country *</span><select name="catalog_country_id" required data-country-select><option value="">Select country</option>@foreach($countries as $country)<option value="{{ $country->id }}" data-eu="{{ $country->is_eu ? '1':'0' }}" data-uefa="{{ $country->is_uefa ? '1':'0' }}">{{ $country->name }} ({{ $country->code }})</option>@endforeach</select></label>
    ...
    <section class="tax-card">
        ...
        <div class="tax-table-wrap" style="margin-top:12px"><table class="tax-table"><thead><tr><th>Menu Item</th><th>Category</th><th>Country</th><th>Club</th><th>Product Type</th><th>Parent</th><th>Public</th></tr></thead><tbody>
            @forelse($categories as $category)
                <tr><td><div class="tax-path"><strong>{{ $category->name }}</strong><small>{{ $category->slug }}</small></div></td><td><span class="tax-pill">{{ $taxonomyTypes[$category->taxonomy_type] ?? str($category->taxonomy_type)->headline() }}</span></td><td>{{ $category->catalogCountry?->name ?? '—' }}</td><td>{{ $category->catalogClub?->name ?? '—' }}</td><td>{{ $productTypes[$category->product_type] ?? '—' }}</td><td>{{ $category->parent?->name ?? 'SHOP ALL' }}</td><td><a href="{{ route('category', ['category'=>$category->slug]) }}" target="_blank" rel="noopener">Open ↗</a></td></tr>
            @empty<tr><td colspan="7"><div class="tax-empty">No taxonomy menu items match these filters.</div></td></tr>@endforelse
        </tbody></table></div>
...
(() => {
    ...
    const type=form.querySelector('[data-taxonomy-select]'); const country=form.querySelector('[data-country-select]'); const club=form.querySelector('[data-club-select]'); const clubField=form.querySelector('[data-club-field]');
    const refreshCountries=()=>{const value=type.value; [...country.options].forEach((o,i)=>{if(i===0)return; const allowed=value==='uefa'?o.dataset.uefa==='1':(['traditional','heritage'].includes(value)?o.dataset.eu==='1':true); o.hidden=!allowed; o.disabled=!allowed;}); if(country.selectedOptions[0]?.disabled)country.value=''; refreshClubs();};
    const refreshClubs=()=>{const value=type.value; const needs=['uefa','fifa','gaa'].includes(value); clubField.hidden=!needs; club.required=needs; club.disabled=!needs; [...club.options].forEach((o,i)=>{if(i===0)return; const allowed=needs&&o.dataset.body===value&&o.dataset.country===country.value; o.hidden=!allowed;o.disabled=!allowed;}); if(club.selectedOptions[0]?.disabled)club.value='';};
    type.addEventListener('change',refreshCountries); country.addEventListener('change',refreshClubs); refreshCountries();
})();
</script>
@endsection
