...
<div class="club-page">
    ...
    <section class="club-card"><h2 style="margin-top:0">Add club</h2><form class="club-add" method="post" action="{{ route('admin.categories.clubs.store') }}" data-club-add-form>@csrf
        <label>Organizations<div class="club-orgs">@foreach($governingBodies as $value=>$label)<label><input type="checkbox" name="organizations[]" value="{{ $value }}" @checked($value === 'english')> {{ $label }}</label>@endforeach</div></label>
