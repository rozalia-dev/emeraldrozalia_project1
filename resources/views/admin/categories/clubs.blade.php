...
<div class="club-page">
    ...
    <section class="club-card"><form class="club-filter" method="get" action="{{ route('admin.categories.clubs') }}"><input type="search" name="q" value="{{ $search }}" placeholder="Search club..."><select name="governing_body"><option value="">All Organizations</option>@foreach($governingBodies as $value=>$label)<option value="{{ $value }}" @selected($governingBody===$value)>{{ $label }}</option>@endforeach</select><select name="catalog_country_id"><option value="">All Countries</option>@foreach($countries as $country)<option value="{{ $country->id }}" @selected($countryId===$country->id)>{{ $country->name }}</option>@endforeach</select><select name="catalog_county_code"><option value="">All Counties</option>@foreach($countyNames as $code=>$name)@if($code)<option value="{{ $code }}" @selected($countyCode===$code)>{{ $name }}</option>@endif @endforeach</select><select name="status"><option value="active" @selected($status==='active')>Active</option><option value="inactive" @selected($status==='inactive')>Inactive</option><option value="all" @selected($status==='all')>All Statuses</option></select><button class="club-btn" type="submit">Filter</button><a class="club-btn" href="{{ route('admin.categories.clubs') }}">Reset</a></form></section>

    <section class="club-card"><h2 style="margin-top:0">Add club</h2><form class="club-add" method="post" action="{{ route('admin.categories.clubs.store') }}" data-club-add-form>@csrf<label>Organization<select name="governing_body" required>@foreach($governingBodies as $value=>$label)<option value="{{ $value }}">{{ $label }}</option>@endforeach</select></label><label>Country<select name="catalog_country_id" required data-club-country><option value="">Select country</option>@foreach($countries as $country)<option value="{{ $country->id }}" data-code="{{ $country->code }}">{{ $country->name }} ({{ $country->code }})</option>@endforeach</select></label><label>County<select name="catalog_county_code" required data-club-county><option value="">Select country first</option></select></label><label>Club Name<input name="name" maxlength="180" required></label><label>Slug<input name="slug" maxlength="220" placeholder="auto-from-name"></label><label>Status<select name="is_active"><option value="1">Active</option><option value="0">Inactive</option></select></label><label>Sort<input type="number" name="sort_order" value="0" min="0" max="100000"></label><button class="club-btn primary" type="submit">Add Club</button></form></section>
    ...
    <section class="club-card"><div style="display:flex;justify-content:space-between;gap:12px;align-items:center"><h2 style="margin:0">Clubs</h2><span class="club-badge">{{ number_format($clubs->total()) }} shown</span></div><div class="club-table-wrap" style="margin-top:12px"><table class="club-table"><thead><tr><th>Organization</th><th>Country</th><th>County</th><th>Club</th><th>Slug</th><th>Status</th><th>Sort</th><th>Menu Uses</th><th>Actions</th></tr></thead><tbody>
        @forelse($clubs as $club)
            ...
            <tr><td><form id="{{ $clubFormId }}" method="post" action="{{ route('admin.categories.clubs.update',$club) }}">@csrf @method('PATCH')</form><select form="{{ $clubFormId }}" name="governing_body">@foreach($governingBodies as $value=>$label)<option value="{{ $value }}" @selected($club->governing_body===$value)>{{ $label }}</option>@endforeach</select></td><td><select form="{{ $clubFormId }}" name="catalog_country_id" data-edit-club-country>@foreach($countries as $country)<option value="{{ $country->id }}" data-code="{{ $country->code }}" @selected($club->catalog_country_id===$country->id)>{{ $country->name }}</option>@endforeach</select></td><td><select form="{{ $clubFormId }}" name="catalog_county_code" data-edit-club-county>@foreach($countyOptionsByCountry as $countryCode=>$rows)@foreach($rows as $row)<option value="{{ $row['code'] }}" data-country-code="{{ $countryCode }}" @selected(strtoupper((string)$club->catalog_county_code)===strtoupper((string)$row['code']))>{{ $row['name'] }}</option>@endforeach @endforeach</select></td><td><input form="{{ $clubFormId }}" class="name" name="name" value="{{ $club->name }}" maxlength="180" required></td><td><input form="{{ $clubFormId }}" class="slug" name="slug" value="{{ $club->slug }}" maxlength="220"></td><td><select form="{{ $clubFormId }}" name="is_active"><option value="1" @selected($club->is_active)>Active</option><option value="0" @selected(!$club->is_active)>Inactive</option></select></td><td><input form="{{ $clubFormId }}" type="number" name="sort_order" value="{{ $club->sort_order }}" min="0" max="100000" style="width:75px"></td><td>{{ number_format($club->categories_count) }}</td><td><div class="club-actions"><button form="{{ $clubFormId }}" class="club-btn" type="submit">Save</button><form method="post" action="{{ route('admin.categories.clubs.destroy',$club) }}" onsubmit="return confirm('Remove this club from Club Master?')">@csrf @method('DELETE')<button class="club-btn danger" type="submit">Delete</button></form></div></td></tr>
...
(() => {
    ...
    document.querySelectorAll('[data-edit-club-country]').forEach(countrySelect => {
        ...
        const sync = () => {
            ...
            [...countySelect.options].forEach(option => {
                const visible = option.dataset.countryCode === countryCode;
                option.hidden = !visible;
                option.disabled = !visible;
            });
            if (countySelect.selectedOptions[0]?.disabled) {
                countySelect.value = [...countySelect.options].find(option => !option.disabled)?.value || '';
            } else {
