...
@php
    ...
    $editorRegions = old('regions');
    $editorRegions = is_array($editorRegions) ? $editorRegions : $selectedRegions;
    ...
    <div class="layout-layout">
        <main>
            <section class="layout-card">
                <div class="layout-card-heading"><div><h2>{{ $selected ? 'Edit layout version '.$selected->version : 'Create the first layout draft' }}</h2><p>Use the structured controls below to edit approved public regions. Save creates an auditable version.</p></div>@if($selected)<span class="layout-status layout-status--{{ $selected->status }}">{{ $statusLabel($selected->status) }}</span>@endif</div>
                @if($selected && $selected->status === 'draft')
                    <form class="layout-editor" method="post" action="{{ route('admin.pages.layouts.update', $selected) }}" data-layout-visual-form>
                        @csrf @method('PATCH')
                        ...
                        @include('admin.pages.partials.layout-fields', ['editorRegions' => $editorRegions, 'defaults' => $defaults])
                ...
                @elseif(!$selected)
                    <form class="layout-editor" method="post" action="{{ route('admin.pages.layouts.store') }}" data-layout-visual-form>
                        @csrf
...
<script>
(() => {
    const editor = document.querySelector('[data-layout-visual-editor]');
    if (!editor) return;
    ...
    editor.querySelectorAll('[data-layout-add]').forEach((button) => button.addEventListener('click', () => {
        ...
        const list = editor.querySelector(`[data-layout-list="${key}"]`);
        const template = editor.querySelector(`[data-layout-template="${key}"]`);
    ...
    const addFooterLink = (button) => {
        const column = button.dataset.layoutAddLink;
        const list = editor.querySelector(`[data-layout-link-list="${column}"]`) || button.previousElementSibling;
        const template = editor.querySelector(`[data-layout-link-template="${column}"]`);
        if (!list || !template) return;
        const index = nextIndex(list);
        const wrapper = document.createElement('div');
        wrapper.appendChild(template.content.cloneNode(true));
        wrapper.innerHTML = replaceTokens(wrapper.innerHTML, {'__LINK_INDEX__': index});
        const row = wrapper.firstElementChild;
        if (row) { list.appendChild(row); bindEditor(row); row.querySelector('input')?.focus(); }
    };
    editor.addEventListener('click', (event) => {
        const button = event.target.closest('[data-layout-add-link]');
        if (button) addFooterLink(button);
    });
})();
</script>
@endpush
