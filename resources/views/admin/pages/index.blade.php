...
@php
    ...
    $displayType = fn ($page) => $page->isHomepage() ? 'Homepage' : match ($page->template) {
...
<div class="pages-screen" data-pages-screen>
    ...
    <div class="pages-kpis" aria-label="Page statistics">
        <a class="pages-kpi-card pages-kpi-card--green" href="{{ route('admin.pages', ['tab' => 'all']) }}"><span class="pages-kpi-icon"><x-icon name="file-text" size="21" /></span><span><small>Total Pages</small><strong>{{ number_format($stats['total']) }}</strong><em>Current inventory</em></span></a>
        <a class="pages-kpi-card pages-kpi-card--purple" href="{{ route('admin.pages', ['tab' => 'published']) }}"><span class="pages-kpi-icon"><x-icon name="file-text" size="21" /></span><span><small>Published</small><strong>{{ number_format($stats['published']) }}</strong><em>Live count</em></span></a>
        <a class="pages-kpi-card pages-kpi-card--orange" href="{{ route('admin.pages', ['tab' => 'drafts']) }}"><span class="pages-kpi-icon"><x-icon name="clock" size="21" /></span><span><small>Drafts</small><strong>{{ number_format($stats['drafts']) }}</strong><em>Needs editorial action</em></span></a>
        <a class="pages-kpi-card pages-kpi-card--blue" href="{{ route('admin.pages', ['tab' => 'hidden']) }}"><span class="pages-kpi-icon"><x-icon name="eye" size="21" /></span><span><small>Hidden</small><strong>{{ number_format($stats['hidden']) }}</strong><em>Live count</em></span></a>
        <a class="pages-kpi-card pages-kpi-card--teal" href="{{ route('admin.pages', ['tab' => 'published']) }}"><span class="pages-kpi-icon"><x-icon name="chart" size="21" /></span><span><small>Page Views (All Time)</small><strong>{{ number_format($stats['page_views']) }}</strong><em>Tracked views only</em></span></a>
    </div>
                ...
                <table class="pages-table">
                    ...
                    @forelse($pages as $page)
                        @php
                            ...
                            $pageViews = $analyticsViews;
                            $updatedBy = data_get($meta, 'updated_by') ?: 'Not recorded';
                        @endphp
                        <tr data-page-row="{{ $page->id }}">
                            ...
                            <td><a class="pages-page-cell" href="{{ $page->trashed() ? route('admin.pages', ['tab' => 'trash']) : route('admin.pages.edit', $page) }}"><span class="pages-thumbnail pages-thumbnail--empty" aria-hidden="true"><x-icon name="file-text" size="20" /></span><span><strong>{{ $page->title }}</strong><small>{{ $page->intro ?: 'Managed website content' }}</small></span></a></td>
                            ...
                            <td><a class="pages-url" href="{{ $page->trashed() ? route('admin.pages', ['tab' => 'trash']) : route('admin.pages.preview', $page) }}" target="_blank">{{ $page->routePath() }} <x-icon name="arrow-right" size="13" /></a></td>
                            <td><strong class="pages-date-value">{{ $page->updated_at?->format('d M Y') ?: 'Not recorded' }}</strong><small>by {{ $updatedBy }}</small></td>
    ...
    @if($activePage)
        <section class="pages-inspector" id="page-builder" aria-labelledby="page-inspector-heading">
            ...
            <div class="pages-inspector-grid">
                ...
                <article class="pages-inspector-card pages-preview-card"><h3>Page Preview</h3><div class="pages-preview-image"><x-icon name="file-text" size="24" /><span>{{ strtoupper(Str::limit($activePage->title, 18, '')) }}</span></div><div class="pages-preview-footer"><small>{{ $activePage->routePath() }}</small><a href="{{ route('admin.pages.preview', $activePage) }}" target="_blank">Preview Page <x-icon name="arrow-right" size="13" /></a></div></article>
            </div>
        </section>
