...
<div class="settings-page" data-settings-root>
    <div class="settings-breadcrumb"><a href="{{route('admin.dashboard')}}">Project 1 Control Panel</a><span>›</span><a href="{{$settingsUrl()}}">Settings</a><?php if (!$isOverview): ?><span>›</span><strong>{{$category['title']}}</strong><?php endif; ?></div>

    <header class="settings-heading">
        <div class="settings-heading-copy">
            <span class="settings-heading-icon {{!$isOverview ? 'settings-tone-'.$category['tone'] : ''}}"><x-icon name="{{$isOverview ? 'settings' : $category['icon']}}" size="24" /></span>
    ...
    <?php if ($isOverview): ?>
        <div class="settings-overview-toolbar">
            <form method="GET" action="{{$settingsUrl()}}" class="settings-filter-form" data-settings-search-form>
                ...
                <label class="settings-select"><span>Status</span><select name="status" onchange="this.form.submit()"><option value="">All statuses</option><option value="configured" {{ $status === 'configured' ? 'selected' : '' }}>Configured</option><option value="attention" {{ $status === 'attention' ? 'selected' : '' }}>Needs attention</option><option value="not-configured" {{ $status === 'not-configured' ? 'selected' : '' }}>Not configured</option></select></label>
            </form>
        ...
        <div class="settings-overview-grid">
            ...
            <aside class="settings-overview-rail">
                <section class="settings-card settings-health-card">
                    <div class="settings-card-heading"><div><h2>System Health</h2><p>Configuration readiness</p></div><span class="settings-status-pill settings-status-pill--{{ $overviewStats['not_configured'] ? 'warning' : 'healthy' }}"><i></i> {{ $overviewStats['not_configured'] ? 'Setup required' : 'Healthy' }}</span></div>
                    @php($readiness = $overviewStats['total'] ? round(($overviewStats['configured'] / $overviewStats['total']) * 100) : 0)
                    <div class="settings-health-score"><div class="settings-donut settings-donut--health" style="--settings-readiness:{{$readiness}}%"><span><strong>{{$readiness}}%</strong><small>Ready</small></span></div><div class="settings-health-lines"><span><i class="settings-dot settings-dot--green"></i>Configured <b>{{$overviewStats['configured']}}</b></span><span><i class="settings-dot settings-dot--orange"></i>Attention <b>{{$overviewStats['attention']}}</b></span><span><i class="settings-dot settings-dot--red"></i>Missing <b>{{$overviewStats['not_configured']}}</b></span></div></div>
                    <div class="settings-mini-progress"><span><b>Configuration readiness</b><strong>{{$readiness}}%</strong></span><i><em style="width:{{$readiness}}%"></em></i></div>
                </section>
                <section class="settings-card">
                    ...
                    <div class="settings-activity-list">
                        <?php if (count($recentActivity) === 0): ?><div class="settings-empty settings-empty--small">No settings changes yet.</div><?php else: ?><?php foreach ($recentActivity as $entry): ?><div class="settings-activity"><i class="settings-activity-mark settings-activity-mark--{{$entry['tone']}}"></i><span><strong>{{$entry['action']}}</strong><small>{{$entry['user']}} · {{$entry['date']}}</small></span></div><?php endforeach; ?><?php endif; ?>
                    </div>
        ...
        <div class="settings-bottom-grid">
            <section class="settings-card settings-summary-card"><div class="settings-card-heading"><div><h2>Configuration Summary</h2><p>Coverage across all settings areas</p></div><span class="settings-heading-count">{{$overviewStats['total']}} total</span></div><div class="settings-summary-bars"><?php foreach (array_slice(array_values($catalog), 0, 8) as $item): ?><div><span><b>{{$item['title']}}</b><small>{{$item['configured']}} / {{$item['count']}}</small></span><i><em class="settings-bar--{{$item['tone']}}" style="width:{{round(($item['configured'] / max(1,$item['count'])) * 100)}}%"></em></i></div><?php endforeach; ?></div></section>
            <section class="settings-card"><div class="settings-card-heading"><div><h2>Settings Health Snapshot</h2><p>Derived from saved tenant records</p></div><span class="settings-trend-flat">Current</span></div><div class="settings-snapshot-copy"><strong>{{$overviewStats['configured']}} of {{$overviewStats['total']}} settings have saved values.</strong><span>{{$overviewStats['not_configured']}} remain unconfigured; no synthetic health trend is shown.</span></div></section>
        </div>
    <?php else: ?>
        ...
        <div class="settings-detail-layout">
            <section class="settings-detail-main">
                    ...
                    <div class="settings-card settings-form-card">
                        ...
                        <div class="settings-form-grid">
                            <?php foreach ($fields as $field): ?>
                                <?php $fieldValue = $value($field['key']); ?>
                                <?php if ($field['type'] === 'boolean'): ?>
                                    <label class="settings-toggle-field"><span><strong>{{$field['label']}}</strong><?php if ($field['help']): ?><small>{{$field['help']}}</small><?php endif; ?></span><input type="hidden" name="{{$field['key']}}" value="0"><input type="checkbox" name="{{$field['key']}}" value="1" {{ (bool) $fieldValue ? 'checked' : '' }}><i aria-hidden="true"></i></label>
                                <?php else: ?>
                                    <label class="settings-field {{($field['wide'] ?? false) ? 'settings-field--wide' : ''}}"><span>{{$field['label']}}</span>
                                        <?php if ($field['type'] === 'select'): ?>
                                            <select name="{{$field['key']}}"><?php foreach ($field['options'] as $optionValue => $optionLabel): ?><option value="{{$optionValue}}" {{ (string) $fieldValue === (string) $optionValue ? 'selected' : '' }}>{{$optionLabel}}</option><?php endforeach; ?></select>
                                        <?php elseif ($field['type'] === 'textarea'): ?>
                                            <textarea name="{{$field['key']}}" rows="3">{{is_scalar($fieldValue) ? $fieldValue : ''}}</textarea>
                                        <?php else: ?>
                                            <input type="{{$field['type']}}" name="{{$field['key']}}" value="{{is_scalar($fieldValue) ? $fieldValue : ''}}" {{ $field['type'] === 'number' ? 'min="0" step="1"' : '' }}>
                                        <?php endif; ?>
                                        <?php if ($field['help']): ?><small>{{$field['help']}}</small><?php endif; ?>
                                    </label>
                                <?php endif; ?>
                            <?php endforeach; ?>
                        </div>
                ...
                <?php elseif ($section === 'automations'): ?>
                    <section class="settings-card settings-table-card"><div class="settings-card-heading"><div><h2>Workflow Library</h2><p>Event-driven actions are recorded in the automation rules table.</p></div><span class="settings-heading-count">{{count($automations)}} workflows</span></div>
                        ...
                        <div class="settings-table-scroll"><table class="settings-table"><thead><tr><th>Workflow</th><th>Trigger</th><th>Actions</th><th>Last updated</th><th>Status</th><th></th></tr></thead><tbody><?php if (count($automations) === 0): ?><tr><td colspan="6" class="settings-empty">No automation workflows have been created.</td></tr><?php else: ?><?php foreach ($automations as $automation): ?><tr><td><strong>{{$automation->name}}</strong><small>UUID {{data_get($automation,'uuid','not recorded')}}</small></td><td><code>{{$automation->event}}</code></td><td>{{implode(', ', (array) $automation->actions) ?: 'No actions yet'}}</td><td>{{optional($automation->updated_at)->format('d M Y, H:i') ?: 'Not recorded'}}</td><td><span class="settings-status-pill settings-status-pill--{{$automation->enabled ? 'healthy' : 'neutral'}}"><i></i> {{$automation->enabled ? 'Enabled' : 'Paused'}}</span></td><td><form method="POST" action="{{route('admin.settings.automations.toggle',$automation)}}">@csrf<button class="settings-row-button" type="submit">{{$automation->enabled ? 'Pause' : 'Enable'}}</button></form></td></tr><?php endforeach; ?><?php endif; ?></tbody></table></div>
                    </section>
                <?php elseif ($section === 'backup-recovery'): ?>
                    <section class="settings-card settings-table-card"><div class="settings-card-heading"><div><h2>Recent Backups</h2><p>Verified private settings manifests with controlled restore.</p></div><form method="POST" action="{{route('admin.settings.backups.store')}}">@csrf<button class="settings-button settings-button--primary" type="submit"><x-icon name="download" size="14" /> Run Backup Now</button></form></div><div class="settings-table-scroll"><table class="settings-table"><thead><tr><th>Backup</th><th>Started</th><th>Location</th><th>Size</th><th>Status</th><th>Recovery</th></tr></thead><tbody><?php if (count($backups) === 0): ?><tr><td colspan="6" class="settings-empty">No backup runs have been recorded.</td></tr><?php else: ?><?php foreach ($backups as $backup): ?><tr><td><strong>{{$backup->type}} backup</strong><small>UUID {{$backup->uuid ?? 'not recorded'}}</small></td><td>{{optional($backup->started_at)->format('d M Y, H:i') ?: 'Not recorded'}}</td><td>{{\Illuminate\Support\Str::limit($backup->location ?? 'Not recorded', 34)}}</td><td>{{$prettyBytes($backup->size_bytes ?? 0)}}</td><td><span class="settings-status-pill settings-status-pill--{{$backup->status === 'completed' && $backup->verified_at ? 'healthy' : 'danger'}}"><i></i> {{$backup->status === 'completed' && $backup->verified_at ? 'Verified' : $statusLabel($backup->status)}}</span></td><td><?php if ($backup->status === 'completed' && $backup->verified_at): ?><form method="POST" action="{{route('admin.settings.backups.restore', $backup)}}" onsubmit="return confirm('Restore this verified settings backup? Current saved settings will be replaced.')">@csrf<input type="hidden" name="confirmation" value="RESTORE"><button class="settings-row-button" type="submit">Restore</button></form><?php else: ?><span class="settings-muted">Unavailable</span><?php endif; ?></td></tr><?php endforeach; ?><?php endif; ?></tbody></table></div><p class="settings-card-note">Restore validates the checksum and format before applying only approved settings fields. Database and uploaded-file recovery remain separate release gates.</p></section>
                <?php elseif ($section === 'integrations'): ?>
                    <section class="settings-card settings-table-card"><div class="settings-card-heading"><div><h2>Connection Health</h2><p>Existing integration records are shown with their current health status.</p></div><a class="settings-card-link" href="{{route('admin.integration-status')}}">Open integration status <x-icon name="arrow-right" size="13" /></a></div><div class="settings-connection-grid"><?php if (count($connections) === 0): ?><div class="settings-empty">No integration connections have been registered.</div><?php else: ?><?php foreach ($connections as $connection): ?><article><span class="settings-connection-icon"><x-icon name="refresh" size="16" /></span><div><strong>{{\Illuminate\Support\Str::headline($connection->service)}}</strong><small>{{$connection->provider ?: 'Provider not configured'}}</small></div><span class="settings-status-pill settings-status-pill--{{$connection->health === 'healthy' ? 'healthy' : ($connection->health === 'degraded' ? 'warning' : 'neutral')}}"><i></i> {{$statusLabel($connection->health)}}</span></article><?php endforeach; ?><?php endif; ?></div></section>
                <?php elseif ($section === 'localization'): ?>
                    <section class="settings-card settings-table-card"><div class="settings-card-heading"><div><h2>Active Languages &amp; Currencies</h2><p>These records are shared by the storefront and Project 1 cPanel.</p></div><span class="settings-heading-count">{{count($languages)}} languages · {{count($currencies)}} currencies</span></div><div class="settings-split-tables"><div><h3>Languages</h3><table class="settings-table"><tbody><?php if (count($languages) === 0): ?><tr><td>No languages configured yet.</td></tr><?php else: ?><?php foreach ($languages as $language): ?><tr><td><strong>{{$language->native_name}}</strong><small>{{$language->locale}}</small></td><td><span class="settings-status-pill settings-status-pill--{{$language->active ? 'healthy' : 'neutral'}}"><i></i> {{$language->active ? 'Active' : 'Inactive'}}</span></td></tr><?php endforeach; ?><?php endif; ?></tbody></table></div><div><h3>Currencies</h3><table class="settings-table"><tbody><?php if (count($currencies) === 0): ?><tr><td>No currencies configured yet.</td></tr><?php else: ?><?php foreach ($currencies as $currency): ?><tr><td><strong>{{$currency->symbol}} {{$currency->code}}</strong><small>{{$currency->name}}</small></td><td><span class="settings-status-pill settings-status-pill--{{$currency->active ? 'healthy' : 'neutral'}}"><i></i> {{$currency->active ? 'Active' : 'Inactive'}}</span></td></tr><?php endforeach; ?><?php endif; ?></tbody></table></div></div></section>
                <?php elseif ($section === 'security-access'): ?>
                ...
                <?php elseif ($section === 'application-settings'): ?>
                ...
                <?php elseif ($section === 'email-notifications'): ?>
                ...
                <?php elseif ($section === 'whatsapp-messaging'): ?>
            ...
            <aside class="settings-detail-rail">
                <section class="settings-card settings-side-card"><div class="settings-card-heading"><div><h2>Section Health</h2><p>{{$category['title']}} readiness</p></div><span class="settings-status-pill settings-status-pill--{{ $category['not_configured'] ? 'warning' : 'healthy' }}"><i></i> {{$category['not_configured'] ? 'Setup required' : 'Healthy'}}</span></div><div class="settings-side-score"><div class="settings-donut settings-donut--{{$category['tone']}}" style="--settings-readiness:{{round(($category['configured'] / max(1, $category['count'])) * 100)}}%"><span><strong>{{round(($category['configured'] / max(1, $category['count'])) * 100)}}%</strong><small>configured</small></span></div><div><b>{{$category['configured']}} of {{$category['count']}}</b><small>settings configured</small><i><em style="width:{{round(($category['configured'] / max(1, $category['count'])) * 100)}}%"></em></i></div></div></section>
                <section class="settings-card settings-side-card"><div class="settings-card-heading"><div><h2>Quick Actions</h2><p>Useful tools for this area</p></div><x-icon name="dots" size="17" /></div><div class="settings-quick-actions"><?php if ($section === 'backup-recovery'): ?><form method="POST" action="{{route('admin.settings.action','run-backup')}}">@csrf<button type="submit"><x-icon name="download" size="14" /> Run backup now</button></form><?php endif; ?><?php if ($section === 'application-settings'): ?><form method="POST" action="{{route('admin.settings.action','clear-cache')}}">@csrf<button type="submit"><x-icon name="refresh" size="14" /> Clear cache</button></form><?php endif; ?><a href="{{$settingsUrl('audit-logs')}}"><x-icon name="file-text" size="14" /> View audit trail</a><form method="POST" action="{{route('admin.settings.action','reset-section')}}">@csrf<input type="hidden" name="section" value="{{$section}}"><button type="submit" data-settings-confirm="Reset this section to its approved defaults?"><x-icon name="rotate-ccw" size="14" /> Reset defaults</button></form></div></section>
