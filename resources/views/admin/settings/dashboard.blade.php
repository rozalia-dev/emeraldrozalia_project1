...
<div class="settings-page" data-settings-root>
    ...
    <?php if ($isOverview): ?>
        ...
        <div class="settings-bottom-grid">
            <section class="settings-card settings-summary-card"><div class="settings-card-heading"><div><h2>Configuration Summary</h2><p>Coverage across all settings areas</p></div><span class="settings-heading-count">186 total</span></div><div class="settings-summary-bars"><?php foreach (array_slice(array_values($catalog), 0, 8) as $item): ?><div><span><b>{{$item['title']}}</b><small>{{$item['configured']}} / {{$item['count']}}</small></span><i><em class="settings-bar--{{$item['tone']}}" style="width:{{round(($item['configured'] / max(1,$item['count'])) * 100)}}%"></em></i></div><?php endforeach; ?></div></section>
