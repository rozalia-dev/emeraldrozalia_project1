    ...
    @if(!$selected || $selected->status === 'draft')
        <section class="cp-card">
            ...
            <form class="cp-form" method="POST" action="{{$selected ? route('admin.settings.cpanel-theme.update',$selected) : route('admin.settings.cpanel-theme.store')}}">
                ...
                <fieldset><legend>cPanel branding</legend><div class="cp-fields">
                    <label class="cp-field"><span>cPanel logo</span><select name="tokens[branding][logo_path]" data-cpanel-logo-select>
                        <option value="/assets/logo/logo_two_line.png" @selected($token('branding.logo_path','/assets/logo/logo_two_line.png')==='/assets/logo/logo_two_line.png')>Approved two-line wordmark</option>
                        <option value="/assets/logo/logo_one_line.png" @selected($token('branding.logo_path')==='/assets/logo/logo_one_line.png')>Approved one-line wordmark</option>
                    </select><small>This changes the logo shown at the top of the cPanel sidebar after activation.</small></label>
                    <label class="cp-field"><span>Logo alt text</span><input name="tokens[branding][logo_alt]" maxlength="180" value="{{old('tokens.branding.logo_alt',$token('branding.logo_alt','Emerald Rozalia Limited'))}}"></label>
                </div></fieldset>
...
(() => {
    ...
    const logoSelect=root.querySelector('[data-cpanel-logo-select]'), logoPreview=root.querySelector('[data-cpanel-logo-preview]');
    logoSelect?.addEventListener('change',()=>{if(logoPreview) logoPreview.src=logoSelect.value;});
})();
</script>
@endpush
