                            ...
                            @forelse($issues->take(5) as $issue)
                                <tr>
                                    ...
                                    <td class="seo-row-actions">
                                        <form method="post" action="{{ route('admin.seo.issue.fix', [$issue['source_type'], $issue['source_id'], $issue['issue_type']]) }}">@csrf<button type="submit">Fix</button></form>
                                        @if($issue['persistent'] && isset($issue['uuid']))<form method="post" action="{{ route('admin.seo.issue.resolve', $issue['uuid']) }}">@csrf<button type="submit" title="Mark as resolved">✓</button></form>@endif
                                    </td>
                                </tr>
                            @empty
