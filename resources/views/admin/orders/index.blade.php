...
<div class="orders-reference-page" data-orders-dashboard>
    ...
    <div class="orders-heading-row">
        ...
        <div class="orders-date-card"><span class="orders-date-icon"><x-icon name="calendar" size="18" /></span><span><b>Date Range</b><strong>{{ $dateRange['from_label'] }}{{ $dateRange['to_label'] ? ' - '.$dateRange['to_label'] : '' }}</strong><small>Live values from PostgreSQL order records</small></span><x-icon name="chevron-down" size="13" /></div>
    </div>
    ...
    <div class="orders-layout">
        <main class="orders-main">
            <section class="orders-kpis" aria-label="{{ $label }} metrics">
                @foreach($metrics as $metric)
                    <article class="orders-kpi">
                        ...
                        <span class="orders-kpi-copy"><b>{{ $metric['label'] }}@if($metric['label'] === 'Order Value') <span class="sr-only">Paid revenue</span>@endif</b><strong>{{ $metric['kind'] === 'money' ? $money($metric['value']) : $integer($metric['value']) }}</strong><small>{{ $metric['source_label'] }}</small></span>
                    </article>
                @endforeach
            </section>
            ...
            <section class="orders-table-card">
                <div class="orders-table-wrap"><table class="orders-table"><thead><tr><th class="orders-check"><input type="checkbox" data-orders-select-all aria-label="Select all orders"></th><th>ORDER #</th><th>{{ $meta['entity_header'] }}</th>@if($meta['entity_mode'] === 'channel')<th>CHANNEL</th>@endif<th>DATE &amp; TIME</th><th>PAYMENT</th><th>AMOUNT</th><th>STATUS</th><th>FULFILLMENT</th><th>ACTIONS</th></tr></thead><tbody>
                    @forelse($orders as $order)
                            @php $address=(array)($order->shipping_address ?: []); $name=data_get($address, 'name') ?: ($order->user?->name ?: 'Guest Customer'); $entity=$meta['entity_mode'] === 'store' ? data_get($address, 'store', $name) : ($meta['entity_mode'] === 'franchise' ? data_get($address, 'franchise', $name).' / '.data_get($address, 'store', 'Store') : ($meta['entity_mode'] === 'buyer' ? data_get($address, 'company', $name) : $name)); $contact=implode(' · ', array_filter([$order->email, $order->phone])); $channel=data_get($address, 'channel', $type === 'buyer' ? 'Marketplace' : 'Website'); @endphp
                    ...
                    @empty
                        <tr><td colspan="{{ $meta['entity_mode'] === 'channel' ? 10 : 9 }}" class="orders-empty"><x-icon name="shopping-bag" size="27" /><strong>{{ $hasLiveData ? 'No orders match these filters' : 'No live orders recorded' }}</strong><span>{{ $hasLiveData ? 'Adjust the filters or create a new '.strtolower($meta['singular']).'.' : $emptyStateMessage }}</span><span class="sr-only">{{ $emptyStateMessage ?: 'No orders match the current filters' }}</span></td></tr>
                    @endforelse
                </tbody></table></div>
                <div class="orders-table-footer"><span>Showing {{ number_format($orders->firstItem() ?? 0) }} to {{ number_format($orders->lastItem() ?? 0) }} of {{ number_format($orders->total()) }} orders</span><div class="orders-pagination">{{ $orders->onEachSide(1)->links() }}</div><label>Rows per page<select name="per_page" data-orders-per-page><option value="8" @selected((int)request('per_page', 8) === 8)>8</option><option value="16" @selected((int)request('per_page', 8) === 16)>16</option><option value="25" @selected((int)request('per_page', 8) === 25)>25</option><option value="50" @selected((int)request('per_page', 8) === 50)>50</option></select></label></div>
            </section>
            ...
            <section class="orders-report-grid">
                ...
                <article class="orders-report-card orders-report-card--products"><div class="orders-card-heading"><h2>Top Selling Products</h2><span>(By Revenue)</span></div><table class="orders-mini-table"><thead><tr><th>#</th><th>PRODUCT</th><th>REVENUE</th><th>QTY SOLD</th></tr></thead><tbody>@forelse($productRows as $row)<tr><td>{{ $loop->iteration }}</td><td>{{ $row['label'] }}</td><td>{{ $money($row['amount']) }}</td><td>{{ $integer($row['quantity']) }}</td></tr>@empty<tr><td colspan="4" class="orders-mini-empty">No product sales data yet.</td></tr>@endforelse</tbody></table><a href="{{ route('admin.resource', 'customer-order-reports') }}">View Products Report <x-icon name="arrow-right" size="13" /></a></article>
                <article class="orders-report-card orders-report-card--entities"><div class="orders-card-heading"><h2>{{ $meta['entity_title'] }}</h2><span>{{ $meta['entity_mode'] === 'channel' ? '(By Orders)' : '' }}</span></div><table class="orders-mini-table"><thead><tr><th>{{ $meta['entity_mode'] === 'channel' ? 'CHANNEL' : $meta['entity_header'] }}</th>@if($meta['entity_mode'] !== 'channel')<th>ORDER VALUE</th>@endif<th>ORDERS</th><th>{{ $meta['entity_mode'] === 'franchise' ? 'STORES' : '%' }}</th></tr></thead><tbody>@forelse($entityRows as $row)<tr><td>{{ $row['label'] }}</td>@if($meta['entity_mode'] !== 'channel')<td>{{ $money($row['amount']) }}</td>@endif<td>{{ $integer($row['orders']) }}</td><td>{{ $meta['entity_mode'] === 'franchise' ? $row['secondary'] : number_format((float)($row['share'] ?? 0), 1).'%' }}</td></tr>@empty<tr><td colspan="{{ $meta['entity_mode'] === 'channel' ? 3 : 4 }}" class="orders-mini-empty">No entity performance data yet.</td></tr>@endforelse</tbody></table><a href="{{ route('admin.sales-reports.dashboard', ['tab' => $meta['entity_mode'] === 'channel' ? 'channels' : 'sales-by-customers']) }}">View Full Report <x-icon name="arrow-right" size="13" /></a></article>
                <article class="orders-report-card orders-report-card--performance"><div class="orders-card-heading"><h2>{{ $meta['performance_title'] }}</h2><span>Live status mix</span></div><div class="orders-performance-list">@forelse($performanceRows as $row)<div><span><i style="--orders-swatch:{{ $row['color'] }}"></i>{{ $row['label'] }}</span><b>{{ $integer($row['value']) }}</b><em>{{ number_format($row['percent'], 1) }}%</em><progress max="100" value="{{ $row['percent'] }}"></progress></div>@empty<div class="orders-mini-empty">No fulfillment data yet.</div>@endforelse</div><a href="{{ route('admin.order-master', array_merge([$type], $queryWithoutTab, ['tab' => 'processing'])) }}">Manage Fulfillment <x-icon name="arrow-right" size="13" /></a></article>
            </section>
        </main>
        ...
        <aside class="orders-rail">
            ...
            <section class="orders-rail-card"><div class="orders-rail-heading"><h2>ORDER NOTIFICATIONS</h2><a href="{{ route('admin.order-master', $type) }}">View All</a></div><div class="orders-notifications">@forelse($notifications as $notice)<div><i class="orders-notice-dot orders-notice-dot--{{ $notice['tone'] }}"></i><span>{{ $notice['text'] }}</span><time>{{ $notice['time'] }}</time></div>@empty<p class="orders-notifications-empty">No order activity has been recorded yet.</p>@endforelse</div></section>
        </aside>
    </div>
</div>
