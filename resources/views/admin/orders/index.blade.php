            ...
            <section class="orders-kpis" aria-label="{{ $label }} metrics">
                @foreach($metrics as $metric)
                    <article class="orders-kpi">
                        ...
                        <span class="orders-kpi-copy"><b>{{ $metric['label'] }}@if($metric['label'] === 'Order Value') <span class="sr-only">Paid revenue</span>@endif</b><strong>{{ $metric['kind'] === 'money' ? $money($metric['value']) : $integer($metric['value']) }}</strong><small><i>↑ {{ $metric['change'] }}</i> vs last 31 days</small></span>
                    </article>
                @endforeach
            </section>
            ...
            <section class="orders-table-card">
                <div class="orders-table-wrap"><table class="orders-table"><thead><tr><th class="orders-check"><input type="checkbox" data-orders-select-all aria-label="Select all orders"></th><th>ORDER #</th><th>{{ $meta['entity_header'] }}</th>@if($meta['entity_mode'] === 'channel')<th>CHANNEL</th>@endif<th>DATE &amp; TIME</th><th>PAYMENT</th><th>AMOUNT</th><th>STATUS</th><th>FULFILLMENT</th><th>ACTIONS</th></tr></thead><tbody>
                    ...
                    @else
                        ...
                        @empty
                            <tr><td colspan="{{ $meta['entity_mode'] === 'channel' ? 10 : 9 }}" class="orders-empty"><x-icon name="shopping-bag" size="27" /><strong>No orders found</strong><span>Adjust the filters or create a new {{ strtolower($meta['singular']) }}.</span><span class="sr-only">No orders in this master</span></td></tr>
                        @endforelse
                    @endif
                </tbody></table></div>
