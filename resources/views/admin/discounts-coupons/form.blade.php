...
<div class="dc-form-page">
    ...
    <form class="dc-form-card" method="post" action="{{ $discount ? route('admin.discounts-coupons.update', $discount) : route('admin.discounts-coupons.store') }}">
        ...
        <div class="dc-form-grid">
            ...
            <div class="dc-field"><label for="value">Discount value</label><input id="value" name="value" type="number" min="0" step="0.01" value="{{ old('value', $discount?->value) }}"><span class="dc-help">For percentage offers, use a value from 0 to 100. Buy X Get Y uses the rule fields below.</span></div>
            ...
            <div class="dc-field"><label for="buy_quantity">Buy quantity (Buy X Get Y)</label><input id="buy_quantity" name="buy_quantity" type="number" min="1" max="1000" step="1" value="{{ old('buy_quantity', data_get($discount?->metadata, 'buy_quantity')) }}"></div>
            <div class="dc-field"><label for="get_quantity">Free quantity (Buy X Get Y)</label><input id="get_quantity" name="get_quantity" type="number" min="1" max="1000" step="1" value="{{ old('get_quantity', data_get($discount?->metadata, 'get_quantity')) }}"></div>
            <div class="dc-field full"><label for="product_skus">Eligible product SKUs (optional)</label><input id="product_skus" name="product_skus" value="{{ old('product_skus', implode(', ', (array) data_get($discount?->metadata, 'product_skus', []))) }}" placeholder="ER-0001, ER-0002"><span class="dc-help">Leave blank for all cart products, or enter a comma/newline-separated SKU allow-list.</span></div>
            <div class="dc-field full"><label for="category_slugs">Eligible category slugs (optional)</label><input id="category_slugs" name="category_slugs" value="{{ old('category_slugs', implode(', ', (array) data_get($discount?->metadata, 'category_slugs', []))) }}" placeholder="baseball-caps, irish-heritage-hats"><span class="dc-help">Buy X Get Y can target products by SKU or category.</span></div>
