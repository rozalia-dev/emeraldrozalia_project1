...
<section class="home-hero">
    ...
    <div class="home-hero-visual">
        ...
        <aside class="try-card">
            ...
            <a class="home-upload-box" href="/virtual-tryon"><span class="home-upload-icon" aria-hidden="true">⌃</span><strong>UPLOAD YOUR PHOTO</strong><small>or</small><span class="home-camera-button">◎ &nbsp; TAKE PHOTO</span></a>
            <a class="btn home-try-button" href="/virtual-tryon">START TRY-ON <span aria-hidden="true">&rarr;</span></a>
            <small>▣ 100% Private &amp; Secure</small>
...
<section class="home-benefits" aria-label="Emerald Rozalia benefits">
    <div><span class="home-benefit-icon" aria-hidden="true">☘</span><b>MADE IN LIMERICK</b><span>Proudly designing &amp; manufacturing in Ireland.</span></div>
    <div><span class="home-benefit-icon" aria-hidden="true">◇</span><b>PREMIUM QUALITY</b><span>Built to last with the finest materials.</span></div>
    <div><span class="home-benefit-icon" aria-hidden="true">♧</span><b>TRADE &amp; BULK ORDERS WELCOME</b><span>Solutions for businesses of all sizes.</span></div>
    <div><span class="home-benefit-icon" aria-hidden="true">▣</span><b>FAST DISPATCH WORLDWIDE</b><span>Reliable delivery across the globe.</span></div>
    <div><span class="home-benefit-icon" aria-hidden="true">◎</span><b>GLOBAL REACH</b><span>Irish roots. Worn everywhere.</span></div>
</section>
    ...
    <div class="home-collection-grid">
        @foreach($homeCollections as $item)
            ...
            <a class="home-collection-card" href="{{$href}}">
                ...
                <div><h3>{{ $item['title'] }}</h3><p>{{ $item['copy'] }}</p><span>SHOP NOW <b aria-hidden="true">&rarr;</b></span></div>
            </a>
        @endforeach
    </div>
</section>
        ...
        @if($newProducts->isNotEmpty())
            @foreach($newProducts->take(6) as $index => $product)
                <a class="home-product-card" href="{{route('product',$product)}}">
                    ...
                    <span>{{ $product->name }}</span><strong>€{{number_format($product->price,2)}}</strong><b class="home-product-cart" aria-hidden="true">♧</b>
                </a>
            @endforeach
        @else
            @foreach($fallbackProducts as $item)
                <a class="home-product-card" href="/shop">
                    ...
                    <span>{{$item['name']}}</span><strong>€{{number_format($item['price'],2)}}</strong><b class="home-product-cart" aria-hidden="true">♧</b>
                </a>
            @endforeach
        @endif
    </div>
