    ...
    <nav data-nav aria-label="Primary">
        @foreach((array) data_get($siteLayoutRegions, 'header.primary_menu', []) as $navItem)
            ...
            @if($navHref)
                ...
                <a class="{{ request()->getPathInfo() === $navPath ? 'is-active' : '' }}" href="{{ $navHref }}" @if(request()->getPathInfo() === $navPath) aria-current="page" @endif>{{ data_get($navItem, 'label') }}</a>
            @endif
        @endforeach
    </nav>
    <div class="utilities" aria-label="Account tools">
        @foreach((array) data_get($siteLayoutRegions, 'header.utility_menu', []) as $utilityItem)
            ...
            @if($utilityHref)
                ...
                <a href="{{ $utilityHref }}" aria-label="{{ $utilityLabel }}" title="{{ $utilityLabel }}"><x-icon name="{{ data_get($utilityItem, 'icon', 'link') }}" size="20" />@if(data_get($utilityItem, 'label') === 'Cart')<small>{{count(session('cart',[]))?'('.array_sum(array_column(session('cart',[]),'quantity')).')':''}}</small>@endif</a>
            @endif
        @endforeach
    </div>
</header>
...
<footer class="site-footer">
    ...
    <div class="footer-brand"><img class="brand-logo-image" src="{{asset($footerLogoPath)}}" alt="{{ data_get($siteBranding, 'legal_name', 'Emerald Rozalia Limited') }}"><p>{{ data_get($siteLayoutRegions, 'footer.brand_description', data_get($siteBranding, 'description', 'Proudly manufacturing hats and caps in Limerick, Ireland.')) }}</p><div class="socials">@foreach((array) data_get($siteLayoutRegions, 'footer.social_links', []) as $social)@php($socialHref = $layoutUrl($social))@if($socialHref)<a href="{{ $socialHref }}" aria-label="{{ data_get($social, 'label', 'Social profile') }}" rel="me noopener" target="_blank"><x-icon name="{{ data_get($social, 'icon', 'link') }}" label="{{ data_get($social, 'label', 'Social profile') }}" /></a>@endif @endforeach</div></div>
    @foreach((array) data_get($siteLayoutRegions, 'footer.columns', []) as $footerColumn)
        <div><h4>{{ data_get($footerColumn, 'title') }}</h4>@foreach((array) data_get($footerColumn, 'links', []) as $footerLink)@php($footerHref = $layoutUrl($footerLink))@if($footerHref)<a href="{{ $footerHref }}">{{ data_get($footerLink, 'label') }}</a>@endif @endforeach@if(data_get($footerColumn, 'title') === 'COMPANY')@foreach($footerPages ?? [] as $footerPage)<a href="{{ route('content.page', ['page' => $footerPage->slug]) }}">{{ $footerPage->title }}</a>@endforeach @endif</div>
    @endforeach
    @php($newsletter = data_get($siteLayoutRegions, 'footer.newsletter', []))
    @php($newsletterHref = $layoutUrl($newsletter))
    <div class="newsletter"><h4>{{ data_get($newsletter, 'title', 'NEWSLETTER') }}</h4><p>{{ data_get($newsletter, 'description', 'Stay updated with new arrivals and offers.') }}</p>@if($newsletterHref)<a class="btn" href="{{ $newsletterHref }}">{{ data_get($newsletter, 'cta_label', 'Contact our team') }} <x-icon name="arrow-right" /></a>@endif<p class="payments">VISA &nbsp; Mastercard &nbsp; PayPal &nbsp; Apple Pay &nbsp; Google Pay</p></div>
    <div class="footer-bottom"><span>{{ data_get($siteBranding, 'footer_text', '© '.now()->year.' Emerald Rozalia Limited. All rights reserved.') }}</span><span><x-icon name="clover" size="14" /> Designed &amp; Manufactured in {{ data_get($siteBranding, 'city', 'Limerick') }}, {{ data_get($siteBranding, 'country', 'Ireland') }}</span><span>@foreach((array) data_get($siteLayoutRegions, 'footer.legal_links', []) as $legalLink)@php($legalHref = $layoutUrl($legalLink))@if($legalHref)<a href="{{ $legalHref }}">{{ data_get($legalLink, 'label') }}</a>@if(!$loop->last) &nbsp; @endif @endif @endforeach</span></div>
</footer>
@stack('scripts')
