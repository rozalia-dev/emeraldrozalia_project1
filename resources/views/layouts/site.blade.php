...
<header class="site-header" data-public-shell-region="header" style="--layout-header-bg: {{ data_get($siteLayoutRegions, 'header.colors.background', '#010705') }}; --layout-header-text: {{ data_get($siteLayoutRegions, 'header.colors.text', '#f4f4ef') }}; --layout-header-accent: {{ data_get($siteLayoutRegions, 'header.colors.accent', '#8cc63e') }};">
    ...
    <div class="utilities" aria-label="Account tools">
        <form method="post" action="/context/language" class="header-context-control header-context-control--language">
            @csrf
            <select name="locale" aria-label="Change language" title="Language" onchange="this.form.submit()">
                @foreach(\App\Models\Language::where('active', true)->orderBy('name')->get() as $l)
                    <option value="{{ $l->locale }}" @selected(($activeLocale ?? app()->getLocale()) === $l->locale)>{{ strtoupper($l->locale) }}</option>
                @endforeach
            </select>
        </form>
        <form method="post" action="/context/currency" class="header-context-control header-context-control--currency">
            @csrf
            <select name="currency" aria-label="Change currency" title="Currency" onchange="this.form.submit()">
