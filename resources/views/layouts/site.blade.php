...
<head>
    ...
    @endif
    <link rel="stylesheet" href="/css/app.css?v=20260919-functional-contact-faq">
                        ...
                        <div class="site-nav-dropdown-menu">
                            @if($isShopMenu)
                                @foreach($catalogNavCategories ?? [] as $catalogCategory)
                                    <a href="{{ route('category', ['category' => $catalogCategory->slug]) }}"><span>{{ $catalogCategory->name }}</span></a>
                                @endforeach
                            @else
                                <a href="{{ $navHref }}">VIEW ALL COLLECTIONS</a>
...
@stack('scripts')
<script src="/js/app.js?v=20260922-public-header-hover"></script>
</body>
</html>
