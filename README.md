...
The current Batch 10 reports-integrity candidate removes fixture-backed totals from the Reports and Sales Reports dashboards, scopes return data to the selected order window, makes empty results explicit, derives report history/filter options from records, and surfaces recorded communication metadata without estimating missing values. See [Batch 10 reports integrity evidence](docs/PROJECT1-BATCH10-REPORTS-INTEGRITY-EVIDENCE.md). PostgreSQL, Blade, container, deployment and browser/accessibility evidence remain subject to the main GitHub Actions gate; the 519-page guide and 167-reference visual acceptance set remain open.
