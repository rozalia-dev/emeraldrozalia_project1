...
The Communication Center contract follow-up now carries tenant/entity links, consent and idempotency state across the shared enquiry path; routes admin replies through an audited after-commit queue boundary; records delivery attempts and safe provider failures; and accepts only signed, duplicate-safe, redacted provider callbacks. Merged release `b8b9db1d1f9313bb3a3f49a3051c38cd321cbbf3` passed PostgreSQL, container/release and Hetzner deployment validation in [run #318](https://github.com/rozalia-dev/emeraldrozalia_project1/actions/runs/34716793864). See [Communication Center contract evidence](docs/PROJECT1-COMMUNICATION-CONTRACT-EVIDENCE.md). Real provider adapters, durable template/approval/follow-up/report contracts, and the 519-page guide/167-reference visual acceptance set remain open.

The next bounded Communication Center slice makes Email Templates durable: the dashboard and `/api/v1/communication/templates` now use a tenant-scoped UUID aggregate with named routes, Form Requests, API Resources, row-lock/version checks, idempotency, lifecycle actions, after-commit events and redacted audit state. Merged release `aef4fc3e42155f5fca76790356bc9e5f0008dc14` passed the full PostgreSQL, container/release and Hetzner deployment workflow in [run #323](https://github.com/rozalia-dev/emeraldrozalia_project1/actions/runs/34718804955). See [Email Templates contract evidence](docs/PROJECT1-COMMUNICATION-TEMPLATE-EVIDENCE.md). Approval, alert, follow-up, provider, report, visual/browser/accessibility and full guide/archive gaps remain open.

The next bounded Communication Center slice makes Email Dashboard durable: `/admin/resource/email` and `/api/v1/communication/email` now use the tenant-scoped UUID-keyed Conversation aggregate with customer/order/message/UID/date search, URL-persisted filters, consent/order context, resolve/reopen/escalate actions, idempotent replies, soft deletion and redacted audit export. Merged release `96da0731a516c4ee652534df44bc2b3c1590ee74` passed the full PostgreSQL, media-browser, container/release and Hetzner deployment workflow in [run #328](https://github.com/rozalia-dev/emeraldrozalia_project1/actions/runs/34720557324). See [Email Dashboard contract evidence](docs/PROJECT1-COMMUNICATION-EMAIL-DASHBOARD-EVIDENCE.md). Approval, alert, follow-up, provider, report, visual/browser/accessibility and full guide/archive gaps remain open.

Batch 10 now removes fixture-backed totals from the Reports and Sales Reports dashboards, scopes return data to the selected order window, makes empty results explicit, derives report history/filter options from records, and surfaces recorded communication metadata without estimating missing values. Merged release `f0dd61f32d3541067787c5b282985ea036ea77a7` passed the full PostgreSQL, container/release and Hetzner deployment workflow in [run #337](https://github.com/rozalia-dev/emeraldrozalia_project1/actions/runs/34724229368); the release backup is `20260912T230453Z-f0dd61f32d35`. See [Batch 10 reports integrity evidence](docs/PROJECT1-BATCH10-REPORTS-INTEGRITY-EVIDENCE.md). The 519-page guide and 167-reference visual acceptance set remain open.

The Banner Management integrity follow-up removes empty-database fixture campaigns, fake counts/clicks/activity and hard-coded KPI comparisons. Banner rows, statistics, activity, UUID traceability and public delivery now remain record-backed, with explicit empty and unavailable states. Deployed release `841e74972e325f7d88f972cc4283125fc11d38b2` passed the PostgreSQL, container/release and Hetzner deployment workflow in [run #339](https://github.com/rozalia-dev/emeraldrozalia_project1/actions/runs/34742279764); the release backup is `20260913T061943Z-841e74972e32`. See [Banner Management integrity evidence](docs/PROJECT1-BANNER-INTEGRITY-EVIDENCE.md). The 519-page guide and 167-reference visual acceptance set remain open.
